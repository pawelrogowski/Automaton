...
console.log(`Extracted ${Object.keys(sequences).length} sequences from production code`);
console.log('\nBreakdown:');
const regionSeqs = Object.keys(sequences).filter(k => !k.startsWith('actionBar_')).length;
const actionSeqs = Object.keys(sequences).filter(k => k.startsWith('actionBar_')).length;
