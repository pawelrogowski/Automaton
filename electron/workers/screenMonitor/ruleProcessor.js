...
class RuleProcessor {
  constructor() {
    ...
    this.PARTY_HEAL_RUNE_ITEMS = new Set([ /* From V1 */
      'ultimateHealingRune',
      'intenseHealingRune',
    ]);
  }
  ...
  async processRules(activePreset, gameState, globalConfig) {
    ...
    if (gameState.attackCdActive && !this.actionTakenThisAttackCooldown && !manaSyncRuleExecutedImmediately && !manaSyncRuleExecutedFromWatch) { // Ensure no other mana sync ran
    ...
    if (!ruleActionTriggeredThisCycle) {
       ...
       if (eligibleRules.length > 0) {
          ...
          if (logExec) console.log(`[RuleProc] Attempting highest priority eligible rule: ${ruleToExecute.id} (Prio: ${ruleToExecute.priority || 0}, Delay: ${ruleToExecute.delay || 0})`);
          ...
          if (nonManaSyncActionSuccess) {
            if (logExec) console.log(`[RuleProc] Successfully initiated action for ${ruleToExecute.id}. It should now be on its delay if applicable.`);
            ruleActionTriggeredThisCycle = true;
          } else {
            if (logExec) console.log(`[RuleProc] Failed to initiate action for ${ruleToExecute.id} (e.g., global cooldown, action error, or conditions no longer met just before execution). It will NOT start its specific delay from this attempt.`);
          }
       } else {
          if (logExec && nonManaSyncPreset.length > 0) console.log("[RuleProc] No non-ManaSync rules met all conditions or were eligible.");
       }
    } else {
        if (logSteps) console.log("[RuleProc] Skipping non-ManaSync evaluation as a ManaSync rule executed or other exclusive action was taken.");
    }

    if (logSteps) console.log(`[RuleProc] --- Cycle End (Action Triggered: ${ruleActionTriggeredThisCycle}, Exclusive CD Action: ${this.actionTakenThisAttackCooldown}) ---`);
  }
  ...
  _filterEligibleRules(now, rules, gameState) {
    ...
    eligibleRules = eligibleRules.filter(rule => { /* PartyHeal Interval Filter */
      if (rule.id.startsWith(this.RULE_PREFIX.PARTY_HEAL)) {
        ...
        if (!intervalMet && logExec) console.log(`[RuleProc] Filter Fail (PartyHeal Interval): ${rule.id} short, ${timeSinceLastHeal}ms < ${this.PARTY_HEAL_MIN_INTERVAL_MS}ms`);
        return intervalMet;
      }
      return true;
    });
    ...
    eligibleRules = eligibleRules.filter(rule => { /* HealFriend Rune Exclusivity */
      if (rule.id.startsWith(this.RULE_PREFIX.PARTY_HEAL) && this.PARTY_HEAL_RUNE_ITEMS.has(rule.actionItem)) {
        if (gameState.attackCdActive) {
            if (this.actionTakenThisAttackCooldown) { /* log & return false */ return false; }
            if (this.healFriendRuneExecutionsThisAttackCooldown >= 2) { /* log & return false */ return false; }
        }
      }
      return true;
    });
    ...
    eligibleRules = eligibleRules.filter(rule => { /* Equip Rule Specifics */
        if (rule.id.startsWith(this.RULE_PREFIX.EQUIP)) {
            ...
            if (rule.equipOnlyIfSlotIsEmpty) {
                ...
                if (rule.targetSlot === 'amulet') expectedEmptyItemKey = 'emptyAmuletSlot';
                else if (rule.targetSlot === 'ring') expectedEmptyItemKey = 'emptyRingSlot';
                else return false;
                if (currentItemInSlot !== expectedEmptyItemKey) return false;
            }
            ...
            if (currentItemInSlot === itemToBeEquippedName) {
                 if (logExec) console.log(`[RuleProc] Equip Rule '${rule.id}': SKIPPING (Avoid Re-equip). Item '${itemToBeEquippedName}' is already equipped in slot '${rule.targetSlot}'.`);
                 return false;
            }
            return true;
        }
    ...
    eligibleRules = eligibleRules.filter(rule => { /* PartyHeal Final Condition */
      if (rule.id.startsWith(this.RULE_PREFIX.PARTY_HEAL)) {
  ...
  _filterRulesNotOnDelay(now, rules) {
    ...
    return rules.filter((rule) => {
      ...
      if (!individualDelayMet) {
        if (logExec) console.log(`[RuleProc] Filter Fail (Individual Delay): ${ruleId}. Time since last: ${timeSinceLastSuccessfulTrigger.toFixed(0)}ms < Delay: ${ruleDelay}ms.`);
        return false;
      }
      ...
      if (rule.id.startsWith(this.RULE_PREFIX.USER) && category) {
        ...
        if (categoryDelay > 0) {
          ...
          if (!categoryDelayMet) {
            if (logExec) console.log(`[RuleProc] Filter Fail (Category Delay ${category}): ${ruleId}. Time since cat last: ${timeSinceCategoryLastTrigger.toFixed(0)}ms < Cat Delay: ${categoryDelay}ms.`);
            return false;
          }
        }
      }
  ...
  _filterRulesByActiveCooldowns(rules, gameState) { /* As before */ 
    return rules.filter((rule) => {
  ...
  _filterRulesByWalkingState(rules, gameState) { /* As before */ 
    const logExec = config.logging.logRuleExecutionDetails;
    return rules.filter((rule) => {
  ...
  _filterRulesByBasicConditions(rules, gameState) { /* As before */ 
    const logExec = config.logging.logRuleExecutionDetails;
    return rules.filter((rule) => {
      let hpMet = true, manaMet = true, monsterMet = true;
      if (rule.id.startsWith(this.RULE_PREFIX.USER) || 
      ...
      if (!allMet && logExec) console.log(`[RuleProc] Filter Fail (Basic Cond): ${rule.id} (H:${hpMet} M:${manaMet} S:${statusMet} Mon:${monsterMet})`);
      return allMet;
    });
  }
  _filterRulesByItemAvailability(rules, gameState) { /* As before, with logging */ 
    const logExec = config.logging.logRuleExecutionDetails;
    return rules.filter(rule => {
      if ( rule.id.startsWith(this.RULE_PREFIX.ACTION_BAR) ||
           rule.id.startsWith(this.RULE_PREFIX.PARTY_HEAL) || // For non-rune party heal
           rule.id.startsWith(this.RULE_PREFIX.EQUIP) ) { // Equip rules click an item on bar
  ...
  _handleAttackCooldownTransitions(now, gameState, activePreset, globalConfig) { /* V1 logic */ 
    const logExec = config.logging.logRuleExecutionDetails;
    ...
    if (attackCdJustStarted) {
      ...
      this.manaSyncWatchList.clear(); this.executedManaSyncThisCooldown.clear(); this.forcedManaSyncExecutedThisCooldown.clear();
      this.actionTakenThisAttackCooldown = false; this.healFriendRuneExecutionsThisAttackCooldown = 0;

      const manaSyncRules = activePreset.filter(r => r.enabled && r.id.startsWith(this.RULE_PREFIX.MANA_SYNC)).sort((a,b) => (b.priority||0)-(a.priority||0));
      for (const rule of manaSyncRules) {
        ...
        if (conditionsMet.all) {
          if (!rule.actionItem) { console.warn(`[RuleProc] ManaSync ${rule.id} no actionItem.`); continue; }
          if (itemIsActive) {
          ...
          } else { this.manaSyncWatchList.set(rule.id, { startTime: now, checkedConditions: conditionsMet }); }
        }
      }
    }
  ...
  _processManaSyncWatch(now, gameState, activePreset, globalConfig) { /* V1 logic, pass 'manaSyncNormal' to _tryExecuteAction */
    const logExec = config.logging.logRuleExecutionDetails;
    ...
    for (const ruleId of sortedWatchKeys) {
      ...
      if (!rule || !rule.actionItem || this.executedManaSyncThisCooldown.has(ruleId) || (now - watchData.startTime > this.MANA_SYNC_WATCH_DURATION_MS)) {
      ...
      if (itemIsNowActive) {
        ...
        if (conditionsStillMet.all) {
          if (!this._hasHigherPriorityEligibleHealFriend(gameState, activePreset, rule.priority || 0, now)) {
            const keypressSent = this._tryExecuteAction(now, rule, gameState, globalConfig, 'manaSyncNormal');
            if (keypressSent) {
              this.executedManaSyncThisCooldown.add(ruleId); this.actionTakenThisAttackCooldown = true; executedFromWatch = true;
            }
          }
        }
  ...
  _processForcedManaSyncExecution(now, gameState, activePreset, globalConfig) { /* V1 logic, pass 'manaSyncForced' to _tryExecuteAction */
    const logExec = config.logging.logRuleExecutionDetails;
    if (!gameState.attackCdActive || !this.attackCooldownStartTime || this.actionTakenThisAttackCooldown) return false;
    const timeSinceCdStart = now - this.attackCooldownStartTime;
    const isInForcedWindow = timeSinceCdStart >= this.MANASYNC_FORCED_EXECUTION_DELAY_MS && timeSinceCdStart <= (this.MANASYNC_FORCED_EXECUTION_DELAY_MS + this.MANASYNC_FORCED_EXECUTION_WINDOW_MS);
    ...
    for (const rule of manaSyncRules) {
      ...
      if (conditionsMet.all) {
        if (!this._hasHigherPriorityEligibleHealFriend(gameState, activePreset, rule.priority || 0, now)) {
          ...
          if (keypressSent) {
            this.forcedManaSyncExecutedThisCooldown.add(rule.id); this.actionTakenThisAttackCooldown = true; return true;
          }
        }
      }
    }
  ...
  _checkManaSyncConditions(rule, gameState) { /* V1 logic */ 
    const hpMet = parseMathCondition(rule.hpTriggerCondition ?? '>=', parseInt(rule.hpTriggerPercentage ?? 0, 10), gameState.hpPercentage);
    const manaMet = parseMathCondition(rule.manaTriggerCondition ?? '<=', parseInt(rule.manaTriggerPercentage ?? 100, 10), gameState.manaPercentage);
    const statusMet = areCharStatusConditionsMet(rule, gameState);
    return { hpMet, manaMet, statusMet, all: hpMet && manaMet && statusMet };
  }
  _hasHigherPriorityEligibleHealFriend(gameState, activePreset, manaSyncPriority, now) { /* V1 logic, ensure healRule delay check uses lastSuccessfulRuleActionTime */
    if (this.actionTakenThisAttackCooldown) return false;
    const competingHealFriends = activePreset.filter(r => r.enabled && r.id.startsWith(this.RULE_PREFIX.PARTY_HEAL) && r.requireAttackCooldown === true && (r.priority || 0) > manaSyncPriority);
  ...
  _processActionConfirmations(now, gameState) { /* As before, just clears pending list */
  ...
  _tryExecuteAction(now, rule, gameState, globalConfig, executionType) {
    ...
    const isPriorityRuleForCooldown = isManaSync || 
    ...
    if (!isPriorityRuleForCooldown && now < this.effectiveCooldownEndTime) {
      if (logExec) console.log(`[RuleProc] Execute REJECTED (Global Cooldown): ${ruleId} (until ${this.effectiveCooldownEndTime}, now ${now})`);
      return false;
    }
    ...
    try {
      if (ruleId.startsWith(this.RULE_PREFIX.PARTY_HEAL) && this.PARTY_HEAL_RUNE_ITEMS.has(rule.actionItem)) {
        ...
        if (targetMember?.uhCoordinates) {
          ...
          useItemOnCoordinates(globalConfig.windowId, targetMember.uhCoordinates.x + getRandomNumber(0,130), targetMember.uhCoordinates.y + getRandomNumber(0,11), rule.key);
          actionSent = true;
        } else {
          if (logExec) console.warn(`[RuleProc] PartyHeal Rune ${ruleId}: No valid target found.`);
        }
      } else if (isManaSync) { // Covers 'manaSyncNormal' and 'manaSyncForced'
        ...
        actionSent = true;
      } else { // 'standard' rules: userRule, equipRule, actionBarItem (non-rune party heal)
  ...
  _shouldHealFriend(rule, gameState) { /* As before */ 
    if (!gameState?.partyMembers || rule.friendHpTriggerPercentage == null) return false;
    if (rule.requireAttackCooldown && !gameState.attackCdActive) return false;
    ...
    if (partyPositionIndex === 0) {
      return gameState.partyMembers.some(m => m.isActive && m.hpPercentage != null && m.hpPercentage > 0 && m.hpPercentage <= hpTriggerPercentage);
    } else {
  ...
  _findPartyHealTarget(rule, gameState) { /* As before */ 
     const hpTriggerPercentage = parseInt(rule.friendHpTriggerPercentage, 10);
    const partyPositionIndex = parseInt(rule.partyPosition, 10);
    ...
    if (partyPositionIndex === 0) {
      const potentialTargets = gameState.partyMembers
          .filter(m => m.isActive && m.hpPercentage != null && m.hpPercentage > 0 && m.hpPercentage <= hpTriggerPercentage)
