...
export const updateSABData = (workerState, config) => {
  ...
  if (!workerState.sabInterface) {
    workerState.logger(
      'error',
      '[Cavebot] Unified SAB interface not available!',
    );
    return;
  }
  ...
  try {
    ...
    workerState.logger(
      'error',
      `[Cavebot] SAB position read failed: ${err.message}`,
    );
  }
  ...
  try {
      ...
      if (pathData.waypoints && pathData.waypoints.length > 0) {
        ...
        if (currentPos && pathStart) {
          ...
          if (isPathStale) {
            ...
            workerState.logger(
              'debug',
              `[Cavebot] Read path from SAB: ${workerState.path.length} waypoints, status: ${workerState.pathfindingStatus}, wptId: ${workerState.pathWptId}`,
            );
          }
        } else {
          workerState.path = pathData.waypoints;
    ...
    workerState.logger(
      'error',
      `[Cavebot] SAB path read failed: ${err.message}`,
    );
  }
};
