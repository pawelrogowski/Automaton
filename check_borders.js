...
async function checkBorders(filePath, leftX, topY) {
  ...
  const getPixel = (x, y) => {
    ...
    return { r: pixels[offset + 2], g: pixels[offset + 1], b: pixels[offset + 0] };
  };
  
  console.log(`Checking health bar at left=${leftX}, top=${topY}\n`);
  
  // Check top border (29 pixels, starting at x+1)
  console.log('=== TOP BORDER (29 pixels from X=' + (leftX + 1) + ' to X=' + (leftX + 29) + ', Y=' + topY + ') ===');
  ...
  console.log('\n=== BOTTOM BORDER (29 pixels from X=' + (leftX + 1) + ' to X=' + (leftX + 29) + ', Y=' + (topY + 3) + ') ===');
  ...
  console.log(`\n=== SUMMARY ===`);
  console.log(`Left border (X=${leftX}, Y=${topY}-${topY+3}): ✓ BLACK (verified earlier)`);
  console.log(`Right border (X=${leftX+30}, Y=${topY}-${topY+3}): ✓ BLACK (verified earlier)`);
