...
async function inspectArea(filePath, centerX, centerY) {
  ...
  console.log(
    `Inspecting area around (${centerX}, ${centerY}) where health bar was expected\n`,
  );
  ...
  console.log(
    `  Left border: X=${expectedLeftX}, Y=${expectedTopY} to ${expectedTopY + 3}`,
  );
  console.log(
    `  Right border: X=${expectedLeftX + 30}, Y=${expectedTopY} to ${expectedTopY + 3}`,
  );
  ...
  for (let y = expectedTopY; y < expectedTopY + 4; y++) {
    ...
    if (!p) {
      ...
      console.log(
        `  Y=${y}: RGB[${p.r}, ${p.g}, ${p.b}] ${isBlack ? '✓ BLACK' : '✗ NOT BLACK'}`,
      );
    }
  }
  ...
  if (!inner) {
    ...
    console.log(
      `  RGB[${inner.r}, ${inner.g}, ${inner.b}] = 0x${color.toString(16).padStart(6, '0').toUpperCase()}`,
    );
  ...
  console.log(
    `Showing area from (${expectedLeftX - 5}, ${expectedTopY - 5}) to (${expectedLeftX + 35}, ${expectedTopY + 8})\n`,
  );
...
if (args.length < 3) {
  ...
  console.log(
    'Example: node inspect_pixels.js /tmp/hb_mismatch_1234.raw 1029 428',
  );
  process.exit(1);
}
