...
async function analyzeFrame(filePath) {
  ...
  console.log(`Data size: ${pixels.length} bytes (expected: ${width * height * 4})`);
  ...
  for (let y = 0; y < height - 3; y++) {
    for (let x = 0; x < width - 31; x++) {
      ...
      if (isBlack(offset0) && isBlack(offset1) && isBlack(offset2) && isBlack(offset3)) {
        ...
        if (isBlack(rightOff0) && isBlack(rightOff1) && isBlack(rightOff2) && isBlack(rightOff3)) {
  ...
  candidates.forEach((c, i) => {
    console.log(`  ${i + 1}. Position: (${c.x}, ${c.y}) | Center: (${c.centerX}, ${c.centerY}) | Color: ${c.color} ${c.rgb}`);
  });
  ...
  if (candidates.length === 0) {
    ...
    for (let y = 0; y < Math.min(height - 3, 100); y++) { // Sample first 100 rows
      for (let x = 0; x < Math.min(width, 500); x++) { // Sample first 500 columns
        const offset0 = (y * width + x) * 4;
        const offset1 = ((y + 1) * width + x) * 4;
        const offset2 = ((y + 2) * width + x) * 4;
        const offset3 = ((y + 3) * width + x) * 4;
        ...
        if (isDark(offset0) && isDark(offset1) && isDark(offset2) && isDark(offset3)) {
          const innerOffset = ((y + 1) * width + (x + 1)) * 4;
          const b = pixels[innerOffset];
          const g = pixels[innerOffset + 1];
          const r = pixels[innerOffset + 2];
          
          if (b > 30 || g > 30 || r > 30) { // Has some color
    ...
    console.log(`Found ${relaxedCandidates.length} dark vertical lines in sample area`);
    if (relaxedCandidates.length > 0) {
      console.log('First 10:');
      relaxedCandidates.slice(0, 10).forEach(c => {
        console.log(`  (${c.x}, ${c.y}) | Next pixel color: ${c.rgb} (sum: ${c.sum})`);
      });
    }
  }
}
...
if (args.length === 0) {
  console.log('Usage: node debug_healthbar_colors.js /tmp/hb_debug_TIMESTAMP.raw');
  process.exit(1);
}
