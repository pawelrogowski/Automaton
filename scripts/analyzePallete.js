...
const PALETTE_OUTPUT_PATH = path.join(
  process.cwd(),
  'resources',
...
async function analyzePalette() {
  ...
  try {
    ...
    const minimapPngFiles = files.filter(
      (file) => file.startsWith('Minimap_Color_') && file.endsWith('.png'),
    );
    ...
    for (const file of minimapPngFiles) {
      ...
      try {
        ...
        if (fileCount % 100 === 0) {
          logger(
            'info',
            `Processed ${fileCount} / ${totalFiles} files. Found ${uniqueColors.size} unique colors so far.`,
          );
        }
      } catch (error) {
    ...
    logger(
      'info',
      `Analysis complete! Found a total of ${colorCount} unique colors.`,
    );
