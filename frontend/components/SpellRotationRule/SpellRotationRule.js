      ...
      <StyledSpellRotationRule className={className}>
        ...
        <div className="rule-controls-top">
          <div
            ...
            title="Enable/Disable this rotation rule"
          >
            <CustomCheckbox
          ...
          <div
            ...
            title="Global hotkey to toggle this rotation (optional modifier)"
          >
            <ListSelect
            ...
            <ListSelect
              ...
              onChange={(e) =>
                handleFieldChange('activationKey', e.target.value)
              }
              className="activation-key-select"
              disabled={isComingSoon}
            >
              {keyboardKeys
                .filter((k) => !['Alt', 'Ctrl', 'Shift'].includes(k.label))
                .map((k) => (
                  <option key={k.value} value={k.value}>
                    {k.label}
                  </option>
          ...
          <div
            className="control-group repeat-toggle-group"
            title="Repeat sequence after the last step?"
          >
            <CustomCheckbox
          ...
          {currentRule.sequence.map((step, index) => (
            <div key={index} className="sequence-step">
              ...
              <ListSelect
                value={step.key}
                onChange={(e) =>
                  handleSequenceChange(index, 'key', e.target.value)
                }
              ...
              <ListInput
                ...
                value={step.delay}
                onChange={(e) =>
                  handleSequenceChange(index, 'delay', e.target.value)
                }
                min="0"
                step="50"
              ...
              <div
                className="control-group left-click-group"
                title="Perform left mouse click after pressing the key?"
              >
                <CustomCheckbox
                  checked={step.leftClick}
                  onChange={(e) =>
                    handleSequenceChange(index, 'leftClick', e.target.checked)
                  }
                  width={16}
                  height={16}
                  ...
                  disabled={isComingSoon}
                />
                <label htmlFor={`leftClick-${currentRule.id}-${index}`}>
                  L.Click
                </label>
              </div>
              <button
        ...
        <div className="rule-actions-bottom">
          <button
            ...
            className="add-step-button rule-button"
            type="button"
            disabled={isComingSoon}
          >
            + Add Step
