...
async function analyzeFrame(filePath, battleListY) {
  ...
  for (let y = searchStartY; y < searchEndY; y++) {
    for (let x = 0; x < width - 31; x++) {
      ...
      if (isBlack(offset0) && isBlack(offset1) && isBlack(offset2) && isBlack(offset3)) {
        ...
        const hasRightBorder = isBlack(rightOff0) && isBlack(rightOff1) && 
                               isBlack(rightOff2) && isBlack(rightOff3);
  ...
  for (let y = searchStartY; y < searchEndY; y++) {
    for (let x = 0; x < width - 31; x++) {
      ...
      if (isNearBlack(offset0) && isNearBlack(offset1) && isNearBlack(offset2) && isNearBlack(offset3)) {
        const innerOffset = ((y + 1) * width + (x + 1)) * 4;
        const b = pixels[innerOffset];
        const g = pixels[innerOffset + 1];
        const r = pixels[innerOffset + 2];
  ...
  if (candidates.length === 0) {
    ...
    candidates.sort((a, b) => a.distanceFromBattleList - b.distanceFromBattleList);
    candidates.slice(0, 10).forEach((c, i) => {
      console.log(`${i + 1}. Position: (${c.x}, ${c.y}) | Right border: ${c.hasRightBorder ? 'YES' : 'NO'} | Inner: ${c.innerColor} ${c.innerRGB} | Distance from battle list: ${c.distanceFromBattleList}px`);
    });
  }
  ...
  if (relaxedCandidates.length === 0) {
    ...
    relaxedCandidates
      ...
      .slice(0, 10)
      .forEach((c, i) => {
        console.log(`${i + 1}. (${c.x}, ${c.y}) | Inner: ${c.rgb} | Left border pixels: ${c.leftBorderRGB.join(', ')}`);
      });
  }
...
if (args.length < 2) {
  ...
  console.log('Example: node analyze_health_bar.js /tmp/hb_mismatch_1234.raw 22');
  process.exit(1);
}
