...
private:
    ...
    // We revert to your original CheckSHM and Connect. They are correct.
    bool CheckSHM() {
    ...
    // THIS IS YOUR ORIGINAL CAPTURE LOOP WITH THE ONE-LINE FIX
    void CaptureLoop() {
        is_capturing = true;
        while (should_capture) {
            ...
            if (img_reply) {
                ...
                // This forces a round-trip to the server, ensuring the shm_get_image
                // operation is complete before we access the shared memory. This is the
                // correct way to synchronize on systems like XWayland that don't send
                // ShmCompletion events.
    ...
    Napi::Value GetLatestFrame(const Napi::CallbackInfo& info) {
        ...
        uint64_t source_size; uint64_t timestamp; uint32_t width, height;
        {
            ...
            timestamp = latest_capture_timestamp_us; width = latest_width; height = latest_height;
        }
        if (targetBuffer.Length() < source_size) { result.Set("success", Napi::Boolean::New(env, false)); return result; }
